...
cand_thor = cand_thor |>
  ...
  dlvad = ifelse(
        ...
        1, 0),
  #HVAD, HM2, HM3
  cont_flow_dlvad = ifelse(CAN_VAD1 %in% c("224", "316", "205", "236", "313", "330") | CAN_VAD2 %in% c("224", "316", "205", "236", "313", "330"), 1, 0)) |> 
  relocate(dlvad, cont_flow_dlvad)
```
...
ADI2020 <- read_csv("2020_ADI_FIPS.csv")
ADI2015 <- read_csv("2015_ADI_FIPS.csv") |> 
  dplyr::select(-FIPS)
ADI2015 <- left_join(ADI2015, dplyr::select(ADI2023, GISJOIN, FIPS), by = "GISJOIN")


ADI_crosswalk_2023 <-read_excel("11_digit_FIPS_to_ZIP_crosswalk.xlsx")
ADI_crosswalk_2020 <-read_excel("TRACT_ZIP_122020.xlsx")
ADI_crosswalk_2015 <-read_excel("tract_zip_122015.xlsx")


assign_adi <- function(adi, crosswalk) {
  adi$FIPS_11_digit <- substr(adi$FIPS, 1, 11)
  adi <- left_join(adi,
                   dplyr::select(crosswalk, TRACT, ZIP),
...
return(as.data.frame(adi))

}

ADI2023 = assign_adi(ADI2023, ADI_crosswalk_2023)
ADI2020 = assign_adi(ADI2020, ADI_crosswalk_2020)
ADI2015 = assign_adi(ADI2015, ADI_crosswalk_2015)

rm(list = c("ADI_crosswalk_2023", "ADI_crosswalk_2020", "ADI_crosswalk_2015"))

cand_zip = left_join(cand_zip, dplyr::select(cand_thor, PX_ID, CAN_LISTING_DT), by = "PX_ID")
cand_zip <- left_join(cand_zip, dplyr::select(ADI2023, mean_adi, ZIP), by = c("ZIP" = "ZIP")) |> 
  rename(adi_2023 = mean_adi)
cand_zip <- left_join(cand_zip, dplyr::select(ADI2020, mean_adi, ZIP), by = c("ZIP" = "ZIP")) |> 
  rename(adi_2020 = mean_adi)
cand_zip <- left_join(cand_zip, dplyr::select(ADI2015, mean_adi, ZIP), by = c("ZIP" = "ZIP")) |> 
  rename(adi_2015 = mean_adi)
cand_zip = cand_zip |> 
  mutate(listing_year = year(CAN_LISTING_DT),
         mean_adi = case_when(
           listing_year %in% c(2014,2015) ~ adi_2015,
           listing_year %in% c(2016, 2017, 2018, 2019) ~ adi_2020,
           listing_year %in% c(2020,2021,2022, 2023,2024) ~ adi_2023
         ))
rm(list = c("ADI2023", "ADI2020", "ADI2015"))
...
cand_thor <- cand_thor %>%
  mutate(mean_adi = as.numeric(as.character(mean_adi)),
         missing_adi = ifelse(is.na(mean_adi), 1,0)) 
...
cand_thor <- cand_thor |> 
  ...
  blood_type = case_when(
    ...
    CAN_ABO %in% c("A1B", "A2B", "AB") ~ "AB",
    CAN_ABO == "B" ~ "B",
    CAN_ABO == "O" ~ "O"),
  blood_type = factor(blood_type,
                      levels = c("O", "A", "AB", "B")),
    initial_status = case_when(
           (CAN_INIT_STAT %in% c('1010', '1020', '1090', '1110', '2010', '2020', '2090', '2110', '3010', '6010', '6011', '6012', '9010') & policy_period == "Post Policy") ~ "Status 1",
           CAN_INIT_STAT %in% c('1030', '1120', '2030', '2120', '6002', '6020', '6030', '9020') & policy_period == "Post Policy" ~ "Status 2",
           (CAN_INIT_STAT %in% c('1130', '2130', '6040') & policy_period == "Post Policy") ~ "Status 3",
           (CAN_INIT_STAT %in% c('1140', '2140', '6004') & policy_period == "Post Policy") ~ "Status 4",
           (CAN_INIT_STAT %in% c('1150', '2150') & policy_period == "Post Policy") ~ "Status 5",
           (CAN_INIT_STAT %in% c('1160', '2160') & policy_period == "Post Policy") ~ "Status 6",
           (CAN_INIT_STAT %in% c('1010', "2010", "6011") & policy_period == "Pre Policy") ~ "Status 1A",
           (CAN_INIT_STAT %in% c('1020', "2020", "6012") & policy_period == "Pre Policy") ~ "Status 1B",
           (CAN_INIT_STAT %in% c('1030', '1120', '2030', '2120', '6002', '6020', '6030', '9020') & policy_period == "Pre Policy") ~ "Old Status 2",
           CAN_INIT_STAT == 2999 ~ "Inactive",
           .default = "Inactive"), 
  initial_status = factor(initial_status,
                          levels = c("Status 1", "Status 2", "Status 3", "Status 4", "Status 5", "Status 6", "Inactive", "Status 1A", "Status 1B", "Old Status 2")),
     COVID = cut(CAN_LISTING_DT,
                 as.Date(c('2014-01-01', '2020-01-01', '2020-07-01', '2021-01-01', "2021-07-01", "2022-01-01", "2022-07-01", '2023-01-01', '2023-07-01', '2024-01-01', '2024-07-01', '2025-01-01')), 
                 labels = c('Pre-COVID', "2020 H1", "2020 H2", "2021 H1", "2021 H2", "2022 H1", "2022 H2", "2023 H1", '2023 H2', '2024 H1','2024 H2')),
  prior_cardiac_surg = ifelse(CAN_CARDIAC_SURG == "Y", 1, 0),
  functional_status = case_when(
    CAN_FUNCTN_STAT %in% c(2100, 2090, 2080) ~ "Good",
    CAN_FUNCTN_STAT %in% c(2070, 2060, 2050) ~ "Moderate",
    CAN_FUNCTN_STAT %in% c(2040, 2030, 2020, 2010) ~ "Poor",
    .default = "Unknown"),
  functional_status = factor(functional_status,
                             levels = c("Good", "Moderate", "Poor", "Unknown")),
  PVR = (CAN_PULM_ART_MEAN - CAN_PCW_MEAN)/CAN_CARDIAC_OUTPUT
)
...
cand_thor <- cand_thor %>%
  ...
  ungroup() %>%
    ...
    outcome_table1_pre = ifelse(!is.na(pre_policy), 
                                case_when(
                                  dlvad == 1 ~ "Waitlisted with BTT-LVAD",
                                  dlvad == 0 ~ "No BTT-LVAD at waitlisting"), NA),
    outcome_table1_post = ifelse(!is.na(post_policy), 
                                 case_when(
                                  dlvad == 1 ~ "Waitlisted with BTT-LVAD",
                                  dlvad == 0 ~ "No BTT-LVAD at waitlisting"), NA))

txt <- gen_text(rep("Heart transplant candidate registrations, 2014-2024", 51641))
...
heart_cohort$CAN_AGE_AT_LISTING[is.na(heart_cohort$CAN_AGE_AT_LISTING)]<- median(heart_cohort$CAN_AGE_AT_LISTING, na.rm=T)
...
```{r}
table1 = heart_cohort |> 
  dplyr::select(CAN_AGE_AT_LISTING, CAN_GENDER, can_race, mean_adi, CAN_ETHNICITY_SRTR, education, insurance, blood_type, CAN_WGT_KG, CAN_HGT_CM, PX_ID, multi_listed, policy_period, dlvad, diagnosis, initial_status, COVID, prior_cardiac_surg, CAN_IV_INOTROP, functional_status, adi_decile_factor, PVR) |> 
  group_by(PX_ID) |> 
  tbl_summary(,
    label = list( 
    can_race ~ "Race",
...
table1_stratified = heart_cohort |> 
  ...
  statistic = list(all_continuous() ~ "{mean} ({sd})", all_categorical() ~ "{n} ({p}%)")) |>  
  modify_table_body(filter, !(variable == "PX_ID" ))|> 
  add_p(test = list(diagnosis ~ "fisher.test", can_race ~ "fisher.test", education ~ "fisher.test", insurance ~ "fisher.test"), test.args = list(diagnosis = list(simulate.p.value = TRUE), can_race = list(simulate.p.value = TRUE), education = list(simulate.p.value = TRUE), insurance = list(simulate.p.value = T)))


table1_supplement_pre = heart_cohort |> 
  dplyr::select(CAN_AGE_AT_LISTING, CAN_GENDER, can_race, mean_adi, CAN_ETHNICITY_SRTR, education, insurance, blood_type, CAN_WGT_KG, CAN_HGT_CM, PX_ID, multi_listed, policy_period, dlvad, diagnosis, initial_status, COVID, prior_cardiac_surg, CAN_IV_INOTROP, functional_status, adi_decile_factor, PVR) |> 
  filter(policy_period == "Pre Policy") |> 
  group_by(PX_ID) |> 
  tbl_summary(by = dlvad,
    label = list( 
    can_race ~ "Race",
    CAN_AGE_AT_LISTING ~ "Age",
    CAN_GENDER ~ "Sex",
  ...
  statistic = list(all_continuous() ~ "{mean} ({sd})", all_categorical() ~ "{n} ({p}%)")) |>  
  modify_table_body(filter, !(variable == "PX_ID" ))|> 
  add_p(test = list(diagnosis ~ "fisher.test", can_race ~ "fisher.test", education ~ "fisher.test", insurance ~ "fisher.test", initial_status ~ "fisher.test"), test.args = list(diagnosis = list(simulate.p.value = TRUE), can_race = list(simulate.p.value = TRUE), education = list(simulate.p.value = TRUE), insurance = list(simulate.p.value = T), initial_status = list(simulate.p.value = T)))

table1_supplement_post = heart_cohort |> 
  dplyr::select(CAN_AGE_AT_LISTING, CAN_GENDER, can_race, mean_adi, CAN_ETHNICITY_SRTR, education, insurance, blood_type, CAN_WGT_KG, CAN_HGT_CM, PX_ID, multi_listed, policy_period, dlvad, diagnosis, initial_status, COVID, prior_cardiac_surg, CAN_IV_INOTROP, functional_status, adi_decile_factor, PVR) |> 
 ...
 table1
 table1_stratified
 table1_supplement_pre
 table1_supplement_post
```
...
summary = centers|>
  ...
  ungroup() |> 
  group_by(period) |> 
  dplyr::summarize(dlvad_num = sum(dlvad_num),
            total_registrations = sum(total_registrations)) |> 
  mutate(
    CAN_LISTING_CTR_CD = "Combined",
    dlvad_percent = dlvad_num / total_registrations) |> 
  ungroup() |> 
  group_by(CAN_LISTING_CTR_CD) |> 
  mutate(
...
arrow_data <- arrow_data |> 
  mutate(
    is_summary = CAN_LISTING_CTR_CD == "Combined") |>
  arrange(
...
arrow_high_adi = arrow_data |> 
  ...
  labs(
    title = "Centers with most disadvantaged candidates",
    x = "Candidates listed with BTT-LVAD (%)", 
  ...
  scale_x_continuous(breaks = c(0, 25, 50, 75), 
                     limits = c(0,75))+
  scale_y_discrete(labels = function(x) { ifelse(x == "Combined", "Combined", "")})
arrow_high_adi
```
...
arrow_low_adi = arrow_data |> 
  ...
  geom_path(aes(group =CAN_LISTING_CTR_CD), 
            arrow = arrow(ends = "first", length = unit(0.1, "inches")), show.legend = FALSE, color = "black")+ 
  labs(
    title = "Centers with least disadvantaged candidates",
    x = "Candidates listed with BTT-LVAD (%)", 
...
arrow_plots_full_no_vol

ggsave("arrow_plot_no_vol.tiff", arrow_plots_full_no_vol, width = 12, height = 3.5)
...
heart_cohort$prior_cardiac_surg <-as.factor(heart_cohort$prior_cardiac_surg) |> relevel(heart_cohort$prior_cardiac_surg, ref = "0")
heart_cohort$CAN_IV_INOTROP <-as.factor(heart_cohort$CAN_IV_INOTROP) |>  relevel(heart_cohort$CAN_IV_INOTROP, ref = "0")
...
full_sens <- glmer(dlvad ~ 
                    transplant_volume  + adi_decile_num + period + adi_decile_num*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = heart_cohort, 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa")) 

sens_no_missing_adi <- glmer(dlvad ~ 
                    transplant_volume  + adi_decile_num + period + adi_decile_num*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = filter(heart_cohort, missing_adi == 0), 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa")) 

sens_continuous_flow_only <- glmer(cont_flow_dlvad ~ 
                    transplant_volume  + adi_decile_num + period + adi_decile_num*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = heart_cohort, 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa"))

full_adi_factor <- glmer(dlvad ~ 
                    transplant_volume  + adi_decile_factor + period + adi_decile_factor*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = heart_cohort, 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa"))
...
full_adi_factor |> tbl_regression(exp = T)
sens_no_missing_adi |>  tbl_regression(exp = T)
sens_continuous_flow_only |>  tbl_regression(exp = T)
```
...
adi_min_estimate = ggplot(prediction_adi_min, aes(x = period, y = Predicted, color = group)) +
  ...
  labs(
    x = "",
    y = "Predicted Probability of BTT-LVAD at Listing",
    title = "Unadjusted"
  ) +
...
prediction_adi_full_sens = estimate_expectation(full_sens, by = c("period", "adi_decile_num", "functional_status = 'Poor'", "initial_status = c('Status 1B', 'Status 4')"))
prediction_adi_full_sens = prediction_adi_full_sens |> filter((period == "Pre-2018 Policy" & initial_status == "Status 1B") | (period == "Post-2018 Policy" & initial_status == "Status 4"))
...
```{r no missing adi sensitivity analysis}
betas_no_missing = fixef(sens_no_missing_adi)
or_adi_post_no_missing= exp(betas_no_missing["adi_decile_num"] +betas_no_missing["adi_decile_num:periodPost-2018 Policy"])
set.seed(1234)
simul <- sim(sens_no_missing_adi, 1000)
...
```{r only continuous flow sensitivity analysis}
betas_cont_flow = fixef(sens_continuous_flow_only)
or_adi_post_cont_flow= exp(betas_cont_flow["adi_decile_num"] +betas_cont_flow["adi_decile_num:periodPost-2018 Policy"])
set.seed(1234)
simul <- sim(sens_continuous_flow_only, 1000)
...
ggsave("adi_pred_plot.tiff", adi_preds, width = 12, height = 4)
```
...
tidy_factor = tidy(full_adi_factor, conf.int = T, effects = "fixed")
tidy_sens_no_missing = tidy(sens_no_missing_adi, conf.int = T, effects = "fixed")
tidy_sens_continuous = tidy(sens_continuous_flow_only, conf.int = T, effects = "fixed")
...
save_df_as_html(tidy_factor, "table_adi_factor.html")
save_df_as_html(tidy_sens_no_missing, "sens_table_no_missing_adi.html")
save_df_as_html(tidy_sens_continuous, "sens_table_continuous_flow_only.html")


```

# Number of listed candidates by decile in each policy period
```{r candidates per decile by policy period}
decile_by_period <- heart_cohort %>%
  group_by(adi_decile_factor, period) %>%   # Group by decile and durable_LVAD
  dplyr::summarize(count = n(), .groups = "drop") %>% # Count occurrences within each group
  as.data.frame() %>%  # Convert to a data frame (optional)
  ungroup() |> 
  dplyr::select(adi_decile_factor, period, count) |> 
  mutate(adi_decile_factor = factor(adi_decile_factor, labels = c("1st\n(Least Disadvantage)", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th","10th\n(Most Disadvantage)")))

# Bar graph  
decile_by_period_plot <- 
  ggplot(decile_by_period, aes(x = adi_decile_factor, y = count, fill = period)) + 
  geom_col(position = "dodge") + 
  labs(
       x = "ADI Decile", 
       y = "Number of Listed Candidates",  
       fill = "Policy Period") + 
  theme_minimal()+
...
ggsave("decile_by_period.tiff", decile_by_period_plot, width = 10, height = 7)
...
heart_cohort = heart_cohort |> 
  mutate(
    PVR = (CAN_PULM_ART_MEAN - CAN_PCW_MEAN)/CAN_CARDIAC_OUTPUT,
    missing_PVR = ifelse(is.na(PVR),1,0)
  )

test = heart_cohort |> filter(missing_PVR == 1)

set.seed(1234)
sens_pvr <- glmer(dlvad ~ 
                    transplant_volume  + adi_decile_num + period + adi_decile_num*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + PVR + (1 | CAN_LISTING_CTR_CD), 
                  data = filter(heart_cohort, missing_PVR == 0), 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa")) 
sens_pvr |> tbl_regression(exp = T)

betas_pvr = fixef(sens_pvr)
or_adi_post_pvr= exp(betas_pvr["adi_decile_num"] +betas_pvr["adi_decile_num:periodPost-2018 Policy"])
set.seed(1234)
simul <- sim(sens_pvr, 1000)
...
ci_adi
p_adi
tidy_pvr = tidy(sens_pvr, conf.int = T, effects = "fixed")
save_df_as_html(tidy_pvr, "table_pvr.html")
```

```{r}
library(rms)

full_adi_rcs <- glmer(dlvad ~ 
                    transplant_volume  + rcs(mean_adi,4) + period + rcs(mean_adi,4)*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = heart_cohort, 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa"))

full_adi_rcs |> tbl_regression(exp = T)


prediction_adi_rcs = estimate_expectation(full_adi_rcs, by = c("period", "mean_adi = c(5,15,25,35,45,55,65,75,85,95)", "functional_status = 'Poor'", "initial_status = c('Status 1B', 'Status 4')")) |> 
  relocate(Predicted)

prediction_adi_rcs = prediction_adi_rcs |> filter((period == "Pre-2018 Policy" & initial_status == "Status 1B") | (period == "Post-2018 Policy" & initial_status == "Status 4"))

#adi - fully adjusted
adi_rcs_estimate = ggplot(prediction_adi_rcs, aes(x = period, y = Predicted, color = mean_adi)) +
  geom_point() +
  geom_path(aes(group =mean_adi))+
  labs(
    x = "",
    y = "Predicted Probability of BTT-LVAD at Listing",
    color = "ADI Percentile",
    title = "Fully Adjusted"
  ) +
  theme_minimal()+
...
ggsave("adi_rcs_plot.tiff", adi_rcs_estimate, width = 5, height = 5)
tidy_rcs = tidy(full_adi_rcs, conf.int = T, effects = "fixed")
save_df_as_html(tidy_rcs, "table_adi_rcs.html")

```

```{r adi multiple imputation}
set.seed(1234)
library(mice)

comparison = comparison |> 
  filter(PX_ID %in% heart_cohort$PX_ID) |> 
  mutate(need_imputation = ifelse(is.na(mean_adi_new) != is.na(mean_adi_orig), 1, 0)) |> 
  distinct(PX_ID, need_imputation, .keep_all = T)
heart_cohort_modelvaronly = heart_cohort |> dplyr::select(transplant_volume  , adi_decile_num , period , competitive_opo, CAN_AGE_AT_LISTING, CAN_GENDER,  can_race , CAN_ETHNICITY_SRTR, CAN_HGT_CM , CAN_WGT_KG , multi_listed , period , education , insurance , diagnosis , blood_type , initial_status , COVID , prior_cardiac_surg , CAN_IV_INOTROP , functional_status, CAN_LISTING_CTR_CD, dlvad, PX_ID)

heart_cohort_modelvaronly = left_join(heart_cohort_modelvaronly, dplyr::select(comparison, PX_ID, need_imputation), by = "PX_ID") |> 
  mutate(adi_decile_num = ifelse(need_imputation == 1, NA, adi_decile_num))
heart_cohort_modelvaronly = heart_cohort_modelvaronly |> dplyr::select(-PX_ID)

test = heart_cohort_modelvaronly |> filter(need_imputation == 1)

imputed_adi = mice(heart_cohort_modelvaronly,m=5,meth='pmm',seed=1234)
imputed_adi = complete(imputed_adi)
sens_impute <- glmer(dlvad ~ 
                    transplant_volume  + adi_decile_num + period + adi_decile_num*period+ transplant_volume*period + competitive_opo+ CAN_AGE_AT_LISTING+ CAN_GENDER+  can_race + CAN_ETHNICITY_SRTR+ CAN_HGT_CM + CAN_WGT_KG + multi_listed + period + education + insurance + diagnosis + blood_type + initial_status + COVID + prior_cardiac_surg + CAN_IV_INOTROP + functional_status + (1 | CAN_LISTING_CTR_CD), 
                  data = imputed_adi, 
                  family = "binomial",
                  control = glmerControl(optimizer = "bobyqa")) 

betas_impute = fixef(sens_impute)
or_adi_post_impute= exp(betas_impute["adi_decile_num"] +betas_impute["adi_decile_num:periodPost-2018 Policy"])
set.seed(1234)
simul <- sim(sens_impute, 1000)
...
tidy_sens_impute = tidy(sens_impute, conf.int = T, effects = "fixed")
save_df_as_html(tidy_sens_impute, "table_multiple_imputation_adi.html")
